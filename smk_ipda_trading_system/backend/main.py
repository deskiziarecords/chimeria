        ...
        try:
            while p.running:
                try:
                    ...
                    traceback.print_exc()
                    try: await ws.send_text(_safe_dumps({"type":"error","message":str(step_err)}))
                    except Exception: pass
                    break
                if result is None:
                    try: await ws.send_text(_safe_dumps({"type": "done"}))
                    except Exception: pass
                    p.running = False
                    break
                try:
                    ...
                    print(f"[SEND] closed during run: {send_err}")
                    p.running = False
                    break
            ...
            print(f"[RUN LOOP] {e}")
            traceback.print_exc()
            try: await ws.send_text(_safe_dumps({"type":"error","message":str(e)}))
            except Exception: pass
        finally:
            p.running = False
