...
# Install JAX numpy shim BEFORE any SMK imports (handles missing jax gracefully)
try:
    _shim_path = os.path.dirname(os.path.abspath(__file__))
    if _shim_path not in sys.path:
        sys.path.insert(0, _shim_path)
    import jax_shim          # noqa — patches jax if missing
    import statsmodels_shim   # noqa — patches statsmodels if missing
except Exception as _se:
    print(f"[SMK] jax_shim warning: {_se}")
...
def _resolve_smk_root() -> str:
    """
    Find the smk_ipda_trading_system root directory.
    Priority:
      1. SMK_DIR environment variable  (set by start.bat / start.sh)
      2. Parent of backend/ dir        (Layout A: backend IS inside SMK root)
      3. Sibling of backend/'s parent  (Layout B: quimeria/backend/ + quimeria/../smk_ipda_trading_system)
    """
    here = os.path.dirname(os.path.abspath(__file__))

    # Priority 1: explicit env var
    env_dir = os.environ.get("SMK_DIR", "").strip()
    if env_dir and os.path.isdir(os.path.join(env_dir, "core")):
        return env_dir

    # Priority 2: parent of backend/ contains core/ (most common: backend IS inside SMK root)
    parent = os.path.dirname(here)
    if os.path.isdir(os.path.join(parent, "core")):
        return parent

    # Priority 3: grandparent contains smk_ipda_trading_system/
    grandparent = os.path.dirname(parent)
    for sibling in ["smk_ipda_trading_system", "smk", "SMK", "chimeria"]:
        candidate = os.path.join(grandparent, sibling)
        if os.path.isdir(os.path.join(candidate, "core")):
            return candidate

    # Priority 4: same level as backend/
    for sibling in ["smk_ipda_trading_system", "smk"]:
