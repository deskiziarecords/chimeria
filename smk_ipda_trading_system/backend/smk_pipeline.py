"""
smk_pipeline.py  —  QUIMERIA SMK bar-by-bar detector pipeline.
Wraps all SMK modules with numpy fallbacks so the server never crashes.
"""
import sys
import os

# ── SHIMS (must run before any SMK import) ───────────────────────────────────
...
def _find_smk_root():
    ...
    # 1. Explicit env var (set by start.bat / start.sh)
    env = os.environ.get("SMK_DIR", "").strip()
    if env and os.path.isdir(os.path.join(env, "core")):
        return env

    # 2. Parent of backend/ dir  (backend IS inside smk_ipda_trading_system)
    ...
    for name in ["smk_ipda_trading_system", "smk", "SMK"]:
        c = os.path.join(grandparent, name)
        if os.path.isdir(os.path.join(c, "core")):
            return c
        c2 = os.path.join(parent, name)
        if os.path.isdir(os.path.join(c2, "core")):
            return c2
...
class SMKPipeline:
    ...
    def _load_modules(self):
        ...
        if bad:
            print(f"[SMK] Partial load — {len(bad)} failed, {len(ok)} OK")
            for e in bad:
                print(f"  x  {e}")
        else:
    ...
    def step(self) -> Optional[Dict]:
        ...
        if len(window) < 3:
            return self._blank_payload(self.raw_bars[idx], idx)

        df = _to_df(window)
        cur = self.raw_bars[idx]
        r: Dict = {"bar": cur, "bar_index": idx, "total_bars": len(self.raw_bars)}
    ...
    def _displacement(self, cur, df):
        ...
        return {"is_disp": body / rng > 0.7, "dir": 1 if cur["close"] > cur["open"] else -1,
                "body_ratio": round(body / rng, 4), "range_mult": 1.0, "vetoed": False, "status": "FALLBACK"}
    ...
    def _harmonic(self, df):
        ...
        return {"phase_diff": round(abs(phi), 3), "inverted": inv, "freq": 0.0,
                ...
                "status": "DISSONANT: λ3 VETO" if inv else "IN_HARMONY"}
    ...
    def _amd(self, r) -> Dict:
        fvg_active   = r["fvg"]["active"]
        disp_active  = r["displacement"]["is_disp"]
        entrapped    = r["vol_decay"]["entrapped"]
        manip_active = r["manipulation"]["active"]
        kl_broken    = not r["kl"]["stable"]
        topo_broken  = r["topology"]["fractured"]
        prev = self.amd_state
        R_MASTER = kl_broken and topo_broken
        if R_MASTER:
            self.amd_state = "Accumulation"
        elif self.amd_state == "Accumulation":
            if entrapped and r["vol_decay"]["stasis"] > 5:
    ...
    def _mandra(self, r) -> Dict:
        m = self.modules.get("mandra")
        if m:
            try:
                phi = np.array([r["fusion"]["p_fused"]])
                t = m.evaluate_gate(current_phi=phi,
                                    stability=r["fusion"]["confidence"], raw_size=0.02)
    ...
    def _blank_payload(self, bar, idx):
...
# ── HELPERS ───────────────────────────────────────────────────────────────────
def _imp(module_path: str, class_name: str):
    import importlib
...
def _to_df(bars: List[Dict]) -> pd.DataFrame:
    ...
    df["atr"] = ((df["high"] - df["low"])
                 .rolling(14).mean()
                 .fillna((df["high"] - df["low"]).mean()))
    df["atr20"] = ((df["high"] - df["low"])
                   .rolling(20).mean()
                   .fillna((df["high"] - df["low"]).mean()))
    return df
